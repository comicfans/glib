...
load("@version_info//:version.bzl", "major_version", "micro_version", "minor_version")
...
glib_ssize_t = ac_cv_sizeof_ssize_t
    ...
    ret += macros.AC_DEFINE_EXPR(
        define = ["long_long_size_equal_to_long_size"],
        expr = """
...
int main () {
  ...
  return 1;
            ...
            name = "int64_t_is_long",
            requires = ["long_long_size_equal_to_long_size"],
        ),
        checks.AC_TRY_COMPILE(
            code = '''
#if defined(_AIX) && !defined(__GNUC__)
...
if ac_cv_sizeof_short==2:
    ...
    ret += macros.AC_DEFINE_EXPR(
        ["gint32", "gint32_modifier", "gint32_format", "guint32_format", "guint32_align"],
        expr = '''
...
if ac_cv_sizeof_int == 8:
    ...
    for t in g_sizet_try_type:
        ret.append(checks.AC_TRY_COMPILE(
            code = """
#include <stddef.h>
...
def gen_glib_conf():
    ...
    langinfo_altmon = checks.AC_TRY_LINK(
        code = """#ifndef _GNU_SOURCE
                                         # define _GNU_SOURCE
                                         #endif
                                         #include <langinfo.h>
                                         int main (int argc, char ** argv) {
    ...
    langinfo_time_codeset = checks.AC_TRY_LINK(
        code = """#include <langinfo.h>
                                               int main (int argc, char ** argv) {
    ...
    functions_checks = [checks.AC_CHECK_FUNC(func, define = define_name(func)) for func in functions] + select({
        "@platforms//os:windows": [],
        "//conditions:default": [
            checks.AC_CHECK_FUNC("if_indextoname", define = define_name("if_indextoname")),
    ...
    header_cond_checks = [
        checks.AC_CHECK_FUNC("statvfs", requires = ["HAVE_SYS_STATVFS_H"]),
...
#include <unistd.h>
    ...
    ] + macros.AC_DEFINE_EXPR(
        define = ["STATFS_ARGS"],
        requires = ["ac_cv_statfs_args_2", "ac_cv_statfs_args_4"],
        expr = """
...
""",
        ...
        "va_val_copy_setting": [
            checks.AC_DEFINE(define = "G_VA_COPY_AS_ARRAY", requires = ["!"]),
        ],
        "//conditions:default": [],
    }) + [
        checks.AC_TRY_COMPILE(code = """
  void some_func (void) {
    ...
    call_a(2,3);
  }""", name = "ac_cv_c99_va_arg_in_macros"),
    ] + macros.AC_DEFINE_EXPR(define = ["ac_cv_check_c99_va_arg_in_macros"], requires = ["ac_cv_c99_va_arg_in_macros"], expr = '''
assert ac_cv_c99_va_arg_in_macros,"GLib requires a C compiler with support for C99 __VA_ARG__ in macros." ''') + [
        checks.AC_DEFINE(define = "GLIB_HAVE_ALLOCA_H", requires = ["ac_cv_header_alloca_h"]),
    ] + select({
        "@platforms//os:windows": [
            checks.AC_DEFINE("THREADS_WIN32", 1),
        ],
        "//conditions:default": [
            checks.AC_DEFINE("THREADS_POSIX", 1),
        ],
    }) + [
        checks.AC_TRY_COMPILE(
...
""".format(pthread_prefix, func),
        )
        for func in ["pthread_attr_setstacksize", "pthread_attr_setinheritsched", "pthread_condattr_setclock", "pthread_cond_timedwait_relative_np", "pthread_getname_np", "pthread_getaffinity_np"]
    ] + [
        checks.AC_TRY_LINK(code = pthread_prefix + '''
                       int main() {
                pthread_setname_np("example");
                return 0;
              }''', name = "ac_cv_pthread_setname_np_without_tid"),
        checks.AC_TRY_LINK(code = pthread_prefix + '''
                int main() {
                  pthread_setname_np(pthread_self(), "example");
                  return 0;
                }
    ''', name = "ac_cv_pthread_setname_np_with_tid"),
        checks.AC_TRY_LINK(code = pthread_prefix + '''
int main() {
                  pthread_setname_np(pthread_self(), "%s", "example");
                  return 0;
                }
    ''', name = "ac_cv_pthread_setname_np_with_tid_and_arg"),
        checks.AC_TRY_LINK(code = pthread_prefix + '''
...
int main() {
  ...
  return 0;
    ...
    ''', name = "ac_cv_have_pthread_set_name_np"),
    ] + macros.AC_DEFINE_EXPR(
        define = ["HAVE_PTHREAD_SETNAME_NP_WITHOUT_TID", "HAVE_PTHREAD_SETNAME_NP_WITH_TID", "HAVE_PTHREAD_SETNAME_NP_WITH_TID_AND_ARG", "HAVE_PTHREAD_SET_NAME_NP"],
        requires = ["ac_cv_have_pthread_set_name_np", "ac_cv_pthread_setname_np_with_tid_and_arg", "ac_cv_pthread_setname_np_with_tid", "ac_cv_pthread_setname_np_without_tid"],
        expr = """
if ac_cv_pthread_setname_np_without_tid:
    HAVE_PTHREAD_SETNAME_NP_WITHOUT_TID = 1
elif ac_cv_pthread_setname_np_with_tid:                          
    HAVE_PTHREAD_SETNAME_NP_WITH_TID = 1
elif ac_cv_pthread_setname_np_with_tid_and_arg:                          
    HAVE_PTHREAD_SETNAME_NP_WITH_TID_AND_ARG = 1
elif ac_cv_pthread_setname_np:
...
'''
        ...
        checks = [
                 ...
                 [
                     ...
                     checks.AC_TRY_COMPILE(code = '''
                                           #include <locale.h>
                                           LC_MESSAGES
                     ''', requires = ["HAVE_LOCALE_H"], define = "HAVE_LC_MESSAGES")
                 ] + ipv6_check
    ...
    poll_checks = [checks.AC_CHECK_HEADER("sys/poll.h"), checks.AC_CHECK_HEADER("sys/types.h")]+[
        checks.AC_COMPUTE_INT(
            ...
            requires = ["ac_cv_header_sys_poll_h", "ac_cv_header_sys_types_h"],
        )
        for tuple in [("POLLIN", "g_pollin"), ("POLLOUT", "g_pollout"), ("POLLPRI", "g_pollpri"), ("POLLERR", "g_pollerr"), ("POLLHUP", "g_pollhup"), ("POLLNVAL", "g_pollnval")]
    ]
...
'''
    ...
    inet_checks = [
        checks.AC_COMPUTE_INT(define = tuple[1], expression = tuple[0], includes = ["#include <sys/types.h>", "#include <sys/socket.h>"])
        for tuple in [
            ("AF_UNIX", "g_af_unix"),
            ("AF_INET", "g_af_inet"),
            ("AF_INET6", "g_af_inet6"),
    ...
    autoconf(
        ...
        checks = [
            ...
            checks.AC_DEFINE(
                "G_HAVE_FREE_SIZED",
                condition = "ac_cv_func_free_sized",
                if_true = True,
                if_false = False,
            ),
...
"""),
                ...
                "//conditions:default": [
                    checks.AC_DEFINE("glib_os", "#define G_OS_UNIX"),
                ],
            },
        ) + select({
